...
exports.chargeRequestMemcached = async function (input) {
    while (true) {
        ...
        var balanceResponse = await getBalanceMemcached(KEY);
        console.timeEnd("checkBalance");

        var remainingBalance = Number(balanceResponse[KEY]);
...
async function chargeMemcached(key, newValue, casToken) {
    return new Promise((resolve, reject) => {
        memcachedClient.cas(key, newValue, casToken, 0, (err, result) => {
            if (err) {
                reject(err);
            }
            else {
